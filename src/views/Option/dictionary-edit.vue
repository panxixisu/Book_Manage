...
<script>
    export default {
        data() {
            return {
                typeOptions:[{category:'dicCategory',label:'书籍分类'},{category:'dicChannel',label:'频道分类'}],
                // dataForm: {
                //     category: '',
        ...
        computed:{
          dataForm:function(){
            return this.$store.state.EditDictionaryData[0]
          }
        },
        methods: {
            submitForm(dataForm) {
                ...
                if (valid) {
                    this.$http.post('web/php/ModifyDictionary.php', this.dataForm).then(res => {
